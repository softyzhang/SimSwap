...
###--- additional to avoid error messages if detsize < 640 ---###
import onnxruntime
...
def btnVideo():
    ...
    #output_filename = video_filename + "(SimSwap).mp4"
        ...
        if  ret:
            ...
            if detect_results is not None:
                ...
                frame = reverse2wholeimage(frame_align_crop_tenor_list,swap_result_list, frame_mat_list, crop_size, frame, logoclass,\
                    os.path.join(temp_results_dir, 'frame_{:0>7d}.jpg'.format(frame_index)),no_simswaplogo,pasring_model =net, norm = spNorm)
            else:
                if not os.path.exists(temp_results_dir):
                    os.mkdir(temp_results_dir)
...
def reverse2wholeimage(b_align_crop_tenor_list,swaped_imgs, mats, crop_size, oriimg, logoclass, save_path = '', \
                    no_simswaplogo = False,pasring_model = None,norm = None):
...
def startup():
    ...
    with torch.no_grad():
        ...
        #except:
            #print ("no face in image detected")
        ...
        #try:
        video_swap(opt.video_path, latend_id, model, app, opt.output_path, temp_results_dir, crop_size)
        cv2.destroyAllWindows()
        tkFenster.deiconify()
...
tkFenster = Tk()
tkFenster.title('SimSwapSingle 0.1 Beta')
